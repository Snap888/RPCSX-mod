...
struct RPCSXApi {
    ...
    bool (*setMotionData)(float accelX, float accelY, float accelZ,
...
struct RPCSXLibrary : RPCSXApi {
    ...
    static std::optional<RPCSXLibrary> Open(const char *path) {
        ...
        // ✅ НОВЫЙ: Загрузка функции setMotionData
        result.setMotionData = reinterpret_cast<decltype(setMotionData)>(dlsym(handle, "_rpcsx_setMotionData"));
        // clang-format on
...
// ==========================================
// ✅ НОВЫЙ: Sixaxis Motion Data JNI обертка
// ==========================================
extern "C" JNIEXPORT jboolean JNICALL
Java_net_rpcsx_RPCSX_setMotionData(
    ...
    // Guard: an older core library may not export this symbol.
    if (!rpcsxLib.setMotionData) return false;
    
    return rpcsxLib.setMotionData(accelX, accelY, accelZ, gyroX, gyroY, gyroZ);
}